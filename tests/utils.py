...
class TestImageHash(unittest.TestCase):
    ...
    def check_hash_algorithm(self, func, image):
        ...
        emsg = "slightly rotated image should have " "similar hash {} {} {}".format(
            original_hash, rotate_hash, distance
        )
        self.assertTrue(distance <= 10, emsg)
        rotate_image = image.rotate(-90)
        rotate_hash = func(rotate_image)
        emsg = "rotated image should have different " "hash {} {}".format(
            original_hash, rotate_hash
        )
        self.assertNotEqual(original_hash, rotate_hash, emsg)
        distance = original_hash - rotate_hash
        emsg = "rotated image should have larger different " "hash {} {} {}".format(
            original_hash, rotate_hash, distance
        )
        self.assertTrue(distance > 10, emsg)
    ...
    def check_hash_stored(self, func, image, sizes=range(2, 21)):
        for hash_size in sizes:
            ...
            emsg = (
                "unexpected hamming distance {}: original hash {} "
                "- stringified hash {}".format(distance, image_hash, other_hash)
            )
